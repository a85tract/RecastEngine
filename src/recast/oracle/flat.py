...
def _declare(argument: dict[str, Any]) -> str:
    spelled = FORTRAN_TYPES.get(str(argument["dtype"]))
    if spelled is None:
...
def fortran_adapter(module: str, plans: list[FlatPlan], reexport: list[str]) -> str:
    ...
    via = {p.subprogram["name"]: p.subprogram.get("public_via") for p in plans}
    reached = {via.get(name) or name for name in {*reexport, *via}}
    lines = [
