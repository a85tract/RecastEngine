...
| **A scanner that could not run does not report a clean scan.** `scan` returns an iterable, so a missing tool and a clean repository are the same value. The suite drives one scanner that ran and found nothing beside one that raised `ScannerUnavailable`, and requires the run to come out `INCOMPLETE` with the two distinguishable. | `test_runner.py` |
| **`incomplete` is neither `skipped` nor `failed`.** Skipped is an optional plugin the operator left out; incomplete is one that is installed, was asked, and could not answer; failed checked something and did not like it. | `test_runner.py` |
| **A wrapped tool's silence is not a clean scan.** A Scanner declaring a `tool` is run against a fake of that name on PATH — clean, garbage, and absent — and must raise `ScannerUnavailable` for the last two. Faking the tool rather than the plugin leaves the argv, the subprocess and the report parsing in the run. | `test_scanner.py` |
| **A recipe that hands anything an executor declares one.** Oracles, Verifiers, Scanners and Adjudicators all take one; a recipe declaring any of them names its executor first, through config. | `test_recipes.py` |
| **A waiver changes the conclusion, not the record.** `allow_incomplete` lets an operator stop counting a named stage's incompleteness. The outcome still says `incomplete (waived)`, and a waiver naming a stage the recipe does not declare is refused rather than ignored. | `test_runner.py` |
