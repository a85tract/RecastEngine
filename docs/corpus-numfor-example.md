...
involved sets the output directory and nothing else. Every command pastes
into a terminal as written, from the repository root, with the project
environment on PATH and a Fortran compiler installed — the oracle stage
compiles the untouched Fortran with f2py.
...
```json
{
  ...
  "reference": { "key": "f2py:basic:6db7377c264c1f3e", "oracle": "f2py-golden" },
  "result": {
    "verdict": "bit_exact",
...
fortran:array_utils
  [ok ] frontend   fortran
  [ok ] transform  translate.numpy             1 deferred block(s)
  [FAIL] verifier   static.rwset                failed: 3/69 blocks disagree: save_array1d/B005, save_array1d/B021, save_array2d/B016
...
The source block reads `unit_`; the translated one does not (`B005` counts
`reshape` the same one-sided way). The gate does
