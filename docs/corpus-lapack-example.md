...
fortran:linalg
  ...
  [FAIL] verifier   static.rwset                failed: 23/229 blocks disagree: deig/B011, deig/B012, deig/B013, zeig/B012, deigvals/B010 (+18 more)
...
subroutine 'X'` and 30 are generic-dispatch refusals. The first group is
...
The verdict lists 23 blocks of 229. The first is a plain disagreement:
...
translation or of the analysis, and the gate does not decide which. The
next two are a different thing:

```json
{"block": "deig/B012", "reads_source_only": ["i_"], "reads_target_only": ["_constants"],
 "writes_source_only": [], "writes_target_only": []}
```

The source reads `i_`, the imaginary unit from `constants`; the target reads
`_constants.i_`, and the check counts the module alias rather than mapping
the attribute back to the name the Fortran used. That is the verifier's
vocabulary being one step short on a use-imported *parameter* spelled
through its companion, and it recurs for every `i_` in the complex
routines. It is not a translation defect, and it is the kind of thing the
corpus exists to surface: eleven libraries that were never CAM, read by
rules that were written against CAM.

Either way the stage fails closed, the oracle build and the differential
never run, and one verdict is recorded instead of three. 206 of 229 blocks
