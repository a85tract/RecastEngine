...
| 10 | A private repository's source reaching a public repository's CI log | job design: only a differential whose output carries no upstream source may run here | **Settled 2026-08-25, when `dump_diff` was the first differential asked to run in CI.** All four migration differentials compare against CESM-language-translator, which is private, while this repository is public — so any of them running on a hosted runner means a private checkout inside a public repository's CI. The checkout itself is containable: repository secrets are withheld from fork-triggered workflows, the token is read-only and `persist-credentials: false`, and the runner is thrown away. **The log is not, and that is what decides which jobs are allowed.** `emit_diff`, `numba_diff` and `cuda_diff` print emitted upstream lines when they disagree, so a single difference publishes upstream source into a world-readable log — they stay out of CI and run from `tools/ci_local.sh`. `dump_diff` prints counts and case labels written in this repository, over cases constructed in this repository, and reaches no upstream text on any path including a difference, so it may run. The rule is the output's content, not the tool's name: a change that makes `dump_diff` echo what upstream parsed retires this row's permission and must say so here. | 2026-08-25 |
